...
/***************************************************************************/	
	...
	function themify_admin_nav() {
		...
		add_submenu_page('themify', $theme['Name'], 'Настройки', 1, 'themify', 'themify_page');
		add_submenu_page('themify', $theme['Name'], 'Документация (АНГ)', 1, 'themify_docs', 'themify_docs');
	}
...
/***************************************************************************/	
	...
	function themify_page() { 
        ...
        <div class="prompt-box">
            <p class="prompt-msg">Введите свою информацию входа в систему Themify, чтобы модернизировать</p>
            <p><label>Имя</label> <input type="text" class="username" /></p>
        ...
        <p id="theme-title"><?php echo $theme['Name']; ?> <em><?php echo $theme['Version']; ?> (<a href="http://themify.me/logs/<?php echo strtolower($theme['Name']); ?>-changelogs" class="themify_changelogs" target="_blank">журналы изменений</a>)</em></p>
		<p class="top-save-btn"><a href="#" id="save-button" class="save-button">Сохранить Все</a></em></p>
		<div id="content">
        
            <!-- nav -->
            <ul id="maintabnav">
                <li class="setting"><a href="#setting">Настройки</a></li>
                <li class="styling"><a href="#styling">Дизайн</a></li>
                <li class="skins"><a href="#skins">Скины</a></li>
                <li class="transfer"><a href="#transfer">Трансфер</a></li>
            </ul>
            <!-- /nav -->
            ...
            <div id="skins" class="maintab">
                <ul class="subtabnav">
                    <li class="selected"><a href="#setting-general">Скины</a></li>
                </ul>
            ...
            <div id="transfer" class="maintab">
                ...
                <ul class="subtabnav">
                    <li><a href="#import-import">Импорт/Экспорт</a></li>
                </ul>
                
                <div id="transfer-import" class="subtab">
                    <p class="biggest-transfer-btn">
                    <a href="#" class="import" id="download-import">Импорт</a> <em>или</em>
                    <a href="#" class="export" id="download-export">Экспорт</a>
                	</p>
                </div>
        ...
        <div id="bottomtab">
            ...
            <p class="reset">
                ...
                <a href="#" id="reset-setting" class="reset-button">Настройки</a><a href="#" id="reset-styling" class="reset-button">Дизайн</a>
            </p>
            <p class="btm-save-btn">
            	<a href="#" class="save-button">Сохранить Все</a>
       		</p>
        </div>
        <!--/footer -->
...
/***************************************************************************/	
	...
	function themify_get_skins(){
		...
		if($data['skin'] == '' || !isset($data['skin'])){
			$output .= '<div class="skin-preview"><a href="#"><img src="'.get_bloginfo('template_directory').'/themify/img/non-skin.gif" alt="skin" id="" class="selected" /></a><br />Нет скина</div>';
		} else {
			$output .= '<div class="skin-preview"><a href="#"><img src="'.get_bloginfo('template_directory').'/themify/img/non-skin.gif" alt="skin" id="" /></a><br />Нет скина</div>';
		}
		// Open Styles Folder
	...
	function themify_get_css(){
		...
		if(is_array($data)){
			...
			$output = "<style type='text/css'>/* Нет значений в базе данных */</style>";
		}
		echo "\n\n".$output;
	}
								...
								if (zip_entry_open($zip,$zip_entry,"r")){
									...
									if ($overwrite === true || $overwrite === false && !is_file($file_name)){
										...
										if(file_exists($file_name)){
											...
											//echo "<span style=\"color:#1da319;\">файл сохранен: </span>".$file_name."<br />";
										} else {
											//echo "<span style=\"color:red;\">файл  не найден: </span>".$file_name."<br />";
										}
									}
		...
		if(function_exists('add_meta_box')){
			foreach($themify_write_panels as $args){
				...
				foreach($pages as $page){
					add_meta_box( 'themify-meta-boxes', 'Пользовательская Панель Themify', 'themify_meta_boxes', trim($page), 'normal', 'high' ); 
				}
			}
		}  
	}  
                              ...
                              <div class="themify_field_row clearfix">
                                    ...
                                    <div class="themify_field">
                                        ...
                                        <span class="themify_upload_buttons">
                                             <a href="#" class="button button-highlighted">Загрузить</a>
                    ...
                    <?php } else if($meta_box['type'] == 'query_category'){ ?>
                             ...
                             <input type="hidden" name="<?php echo $meta_box['name']; ?>_noncename" id="<?php echo $meta_box['name']; ?>'_noncename" value="<?php echo wp_create_nonce( plugin_basename(__FILE__) ); ?>" />
                                ...
                                <div class="themify_field_row clearfix">    
                                    <div class="themify_field_title"><?php echo $meta_box['title']; ?></div>
                                    <div class="themify_field">
                                        <?php echo preg_replace('/>/', '><option></option>', wp_dropdown_categories(array("class"=>"query_category_single","show_option_all"=>"Все Рубрики","hide_empty"=>0, "echo"=>0,"name"=>$meta_box['name'],"selected"=>$meta_value)), 1);
                                        ?> или
	...
	function themify_get_image($args){
		...
		if($src == '' || !isset($src)){
			if($field_name != '' && isset($field_name) && in_the_loop()){
				...
				if(in_the_loop()){
					$fields = array("Функция изображения", "feature_image", "Изображение записи", "post_image", "image");
					foreach($fields as $field){
	...
	function themify_check_version(){
		...
		if($_GET['success'] == "true" && $_GET['upgrade_type'] == "theme"){
			$notifications .= "<p class='success'>Успешно обновлено ".$theme['Name']." до версии ".$theme['Version']."</p>";	
		}
		if($_GET['success'] == "true" && $_GET['upgrade_type'] == "framework"){
			$notifications .= "<p class='success'>Успешно обновлено Themify framework до версии ".$version."</p>";	
		}
			...
			foreach($versions['versions']['_c']['version'] as $update){
				...
				if($update['_a']['name'] == 'themify'){
					if(str_replace(".","",trim($version)) < $latest){
						...
						$notifications .= "<p class='update ".$login."'>Framework версия ".$update['_v']." теперь доступна. <a href='".$url."' title='' class='".$class."' target='".$target."'>Обновить сейчас</a> или просмотреть <a href='http://themify.me/logs/' title='' class='' target='_blank'>журнал изменений</a> для деталей.</p>";	
					}
					...
					if(str_replace(".","",$theme['Version']) < $latest){
						...
						$notifications .= "<p class='update ".$login."'>".$theme['Name']." версия ".$update['_v']." теперь доступна. <a href='".$url."' title='' class='".$class."' target='".$target."'>Обновить сейчас</a> или просмотреть <a href='http://themify.me/logs/' title='' class='' target='_blank'>журнал изменений</a> для деталей.</p>";	
					}
				}
			}
		}
	...
	function themify_shortcode_slider($atts, $content = null)
	{
		...
		{
			// Отключить, автопрокручиванее, если пользователь нажимает предыдущую или следующую кнопку.
			...
			// Пауза, автопрокручивания, если пользователь перемещается курсором через клип.
