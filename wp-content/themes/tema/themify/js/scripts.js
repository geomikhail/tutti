...
jQuery(function($){  
	...
	$("#reset-styling").live('click', function(e){
		e.preventDefault();
		var reply = confirm("Вы уверены, что хотите сбросить стиль темы?");
		if(reply){
			showAlert();
	...
	$("#reset-setting").live('click', function(e){
		e.preventDefault();
		var reply = confirm("Вы действительно хотите сбросить настройки темы?");
		if(reply){
			showAlert();
	...
	$(".upgrade-framework, .upgrade-theme").live('click', function(e){
		e.preventDefault();
		var reply = confirm("Убедитесь, что Вы сделали резервное копирование перед обновлением. Файлы и параметры могут быть потеряны или изменены."), 
			el = $(this);
	...
	$('.preset a span').live("click",function(e){
		e.preventDefault();
		el = $(this);
		var reply = confirm("Вы действительно хотите удалить это изображение навсегда?");
		if(reply){
	...
	$("#download-import").bind({
		"uploadifySelect" : function(event, id, file){
			var reply = confirm("Импорт перезапишет все настройки и конфигурации. Нажмите OK для продолжения, Отмена, чтобы остановить.");
			if(reply){
	...
	function showLogin(status){
		if(status == 'error'){
			if($('.prompt-box .prompt-error').length == 0){
				$('.prompt-box .prompt-msg').after('<p class="prompt-error">Была ошибка, пожалуйста, проверьте свою информацию</p>');
			}
		} else {
